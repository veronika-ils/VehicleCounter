...
def _encode_text_ensemble(model, device, clip, variants):
    tokens = clip.tokenize(variants).to(device)#site varijanti vo tokeni gi pravime
    feats = model.encode_text(tokens)#site tokeni niz clip za da dobieme vektori
    feats = F.normalize(feats, dim=-1)#normalizirame
    feat_mean = feats.mean(dim=0, keepdim=True)#srednata vrednost(feats e vsushnost matrica)
...
def _build_txt_feats(model, device, clip, max_k: int):
    ...
    for k in range(max_k + 1):#za site od 0 do 10 oravi razlicni varijacii na ednoto isto
        ...
        txt_feats.append(_encode_text_ensemble(model, device, clip, variants))#vektori
...
def clip_estimate_cars(pil_img, max_k=10):
    ...
    img_tensor = torch.stack(ims).to(device)#povekje verzii za istata slika
    img_feat = F.normalize(model.encode_image(img_tensor), dim=-1).mean(0, keepdim=True)#vektor->normalizacija->sredna vrendost
    ...
    probs = logits.softmax(dim=1).squeeze(0)#da se verojatsnosti
