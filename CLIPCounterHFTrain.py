...
@torch.no_grad()
def extract_img_feat(pil_img: Image.Image) -> np.ndarray:
...
@torch.no_grad()
def clip_estimate_cars(
    pil_img: Image.Image,
    max_k: int = 10,
    include_more_than_k: bool = True,
    head=None,
    ...
    t_feat = _get_txt_feats_cached(max_k, include_more_than_k)
    ...
    if head is not None:
        ...
        if hasattr(head, "predict_proba"):
            ...
            p_fused = (1.0 - fuse_alpha) * pz + fuse_alpha * probs_head
            ...
            if N == (max_k + 2):
                probs_all = np.concatenate([p_fused, probs_zeroshot[-1:].cpu().numpy()])
            else:
                probs_all = p_fused
...
def main():
    ...
    parser.add_argument("--include_more_than_k", action="store_true", help="Add '>K' class to zero-shot")
    parser.add_argument("--fuse_alpha", type=float, default=0.5, help="Fusion weight for head vs zero-shot (0..1)")
